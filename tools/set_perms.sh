...
function set_perms {
    ...
    executables+=($(echo source/usr/sbin/{sync_wireguard_to_standby,wireguard_client_admin,wireguard_logger}))
    executables+=(source/usr/lib/wireguard-kit/install_wireguard_client)
    executables+=(tools/git-store-meta/git-store-meta.pl)
    executables+=($(echo tools/git-store-meta/hooks-for-*/{post-checkout,post-merge,pre-commit}))
